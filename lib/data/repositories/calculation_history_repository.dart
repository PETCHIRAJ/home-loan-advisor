...
class CalculationHistoryRepositoryImpl implements CalculationHistoryRepository {
  ...
  Future<Either<String, CalculationHistory?>> getHistoryById(String id) async {
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (history) {
  ...
  Future<Either<String, void>> saveHistory(CalculationHistory history) async {
    try {
      final allHistoryResult = await getAllHistory();
      return allHistoryResult.fold((error) => Left(error), (
        existingHistory,
      ) async {
        ...
        final historyJson = json.encode(
          trimmedHistory.map((h) => h.toJson()).toList(),
        );
        await _prefs.setString(_historyKey, historyJson);

        // Save lightweight history items for quick access
        final historyItems = trimmedHistory
            .map((h) => HistoryItem.fromCalculationHistory(h))
            .toList();
        final itemsJson = json.encode(
          historyItems.map((h) => h.toJson()).toList(),
        );
  ...
  @override
  Future<Either<String, void>> updateHistory(
    ...
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (
        ...
        final historyJson = json.encode(
          updatedList.map((h) => h.toJson()).toList(),
        );
        await _prefs.setString(_historyKey, historyJson);

        // Update history items too
        final historyItems = updatedList
            .map((h) => HistoryItem.fromCalculationHistory(h))
            .toList();
        final itemsJson = json.encode(
          historyItems.map((h) => h.toJson()).toList(),
  ...
  Future<Either<String, void>> deleteHistory(String id) async {
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (
        ...
        final updatedHistory = existingHistory
            .where((h) => h.id != id)
            .toList();
        ...
        // Update history items too
        final historyItems = updatedHistory
            .map((h) => HistoryItem.fromCalculationHistory(h))
            .toList();
        final itemsJson = json.encode(
          historyItems.map((h) => h.toJson()).toList(),
  ...
  @override
  Future<Either<String, List<CalculationHistory>>>
  getBookmarkedHistory() async {
    try {
      final allHistoryResult = await getAllHistory();
      return allHistoryResult.fold((error) => Left(error), (history) {
    ...
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (history) {
        ...
        for (final h in history) {
          tenureMap[h.parameters.tenureYears] =
              (tenureMap[h.parameters.tenureYears] ?? 0) + 1;
        }
  ...
  @override
  Future<Either<String, List<CalculationHistory>>> searchHistory(
    String query,
  ) async {
    try {
      final allHistoryResult = await getAllHistory();
      return allHistoryResult.fold((error) => Left(error), (history) {
        ...
        final filtered = history.where((h) {
          // Search in name
          if (h.name != null &&
              h.name!.toLowerCase().contains(lowercaseQuery)) {
            return true;
          }
  ...
  Future<Either<String, String>> exportHistoryAsCSV() async {
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (history) {
        ...
        // CSV Header
        csvData.writeln(
          'Date,Name,Loan Amount,Interest Rate,Tenure Years,Monthly EMI,Total Interest,Total Amount,Tax Savings,PMAY Eligible',
        );
        ...
        for (final h in history) {
          ...
          final taxSavings = h.result.taxBenefits.totalAnnualSavings
              .toStringAsFixed(0);
          final pmayEligible = h.result.pmayBenefit?.isEligible ?? false;

          csvData.writeln(
            '$date,$name,$loanAmount,$interestRate,$tenure,$monthlyEMI,$totalInterest,$totalAmount,$taxSavings,$pmayEligible',
          );
        }
    ...
    try {
      ...
      return allHistoryResult.fold((error) => Left(error), (history) {
        ...
        for (int i = 0; i < history.length; i++) {
          ...
          textData.writeln(
            '   Date: ${h.timestamp.day}/${h.timestamp.month}/${h.timestamp.year}',
          );
          textData.writeln(
            '   Loan Amount: ₹${h.parameters.loanAmount.toStringAsFixed(0)}',
          );
          textData.writeln(
            '   Interest Rate: ${h.parameters.interestRate.toStringAsFixed(2)}%',
          );
          textData.writeln('   Tenure: ${h.parameters.tenureYears} years');
          textData.writeln(
            '   Monthly EMI: ₹${h.result.monthlyEMI.toStringAsFixed(0)}',
          );
          textData.writeln(
            '   Total Interest: ₹${h.result.totalInterest.toStringAsFixed(0)}',
          );
          textData.writeln(
            '   Tax Savings: ₹${h.result.taxBenefits.totalAnnualSavings.toStringAsFixed(0)}',
          );
          if (h.result.pmayBenefit?.isEligible == true) {
            textData.writeln(
              '   PMAY Subsidy: ₹${h.result.pmayBenefit!.subsidyAmount.toStringAsFixed(0)}',
            );
          }
          textData.writeln();
        }
